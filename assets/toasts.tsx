	...
	createEffect(() => {
		...
		interval = setInterval(() => {
			for (const message in lineWidths()) {
				const newWidth = Math.max(lineWidths()[message] - (TOAST_WIDTH / (TOAST_DURATION / UPDATE_INTERVAL)), 0);
		...
		<>
			{messages().map(({ message, type }) => (
				<div 
					...
					class={clsx(
						'absolute bottom-2 break-words overflow-x-hidden rounded-2xl border shadow',
				...
				>
					<div class="flex flex-row items-center">
						<button 
							class="block w-full h-full max-w-[64px] fill-red-500 p-4"
							type="button"
					...
					<div class="bg-red-500 h-[4px] transition-all" style={{ width: `${lineWidths()[message] ?? 300}px`}}></div>
				</div>
			))}
		</>
	);
